...
#
# Todo: Implemente continuous mode with param "-c" to loop even when failing; so that nightly-runs can be facilitated (also add statistics for that)
...
function powerCycle () {
    ...
    while true; do
        # increment
        CYCLES=$((CYCLES+1))
        echo "-------------------- run $CYCLES / $FAILS freezes yet / $(date) --------------------"
